...
for s = 1:size(SampleName_Group,1)
    for g1 = 2:size(SampleName_Group,2)
        if ~isempty(SampleName_Group{s,g1})
            for g2 = 2:size(SampleName_Group,2)
                if ~isempty(SampleName_Group{s,g2})
                    if g1<g2
                        nPairWise = nPairWise+1;
                        Pairwise{nPairWise,1} = SampleName_Group{s,1};
                        Pairwise{nPairWise,2} = SampleName_Group{s,g1};
                        Pairwise{nPairWise,3} = SampleName_Group{s,g2};
                        ...
                        for i = 1:size(NameList_Train,1)
                            for j = 1:size(NameList_Train,1)
                                ...
                                %%%%% same Name; different group; same mode
                                if strcmp(Name1(1:2),SampleName_Group{s,1}) && strcmp(Name2(1:2),SampleName_Group{s,1})
                                    if strcmp(Name1(4:5),SampleName_Group{s,g1}) && strcmp(Name2(4:5),SampleName_Group{s,g2})
